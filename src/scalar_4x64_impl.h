...
SECP256K1_INLINE static unsigned int secp256k1_scalar_get_bits_var(const secp256k1_scalar_t *a, int offset, int count) {
    ...
    if ((offset + count - 1) / 64 == offset / 64) {
        return secp256k1_scalar_get_bits(a, offset, count);
    } else {
        VERIFY_CHECK(offset / 64 + 1 < 4);
        return ((a->d[offset / 64] >> (offset % 64)) | (a->d[offset / 64 + 1] << (64 - (offset % 64)))) & ((((uint64_t)1) << count) - 1);
    }
}
...
static void secp256k1_scalar_add_bit(secp256k1_scalar_t *r, unsigned int bit) {
    ...
    uint128_t t = (uint128_t)r->d[0] + (((uint64_t)((bit / 64) == 0)) << bit);
    r->d[0] = t & 0xFFFFFFFFFFFFFFFFULL; t >>= 64;
    t += (uint128_t)r->d[1] + (((uint64_t)((bit / 64) == 1)) << (bit % 64));
    r->d[1] = t & 0xFFFFFFFFFFFFFFFFULL; t >>= 64;
    t += (uint128_t)r->d[2] + (((uint64_t)((bit / 64) == 2)) << (bit % 64));
    r->d[2] = t & 0xFFFFFFFFFFFFFFFFULL; t >>= 64;
    t += (uint128_t)r->d[3] + (((uint64_t)((bit / 64) == 3)) << (bit % 64));
    r->d[3] = t & 0xFFFFFFFFFFFFFFFFULL;
#ifdef VERIFY
    VERIFY_CHECK((t >> 64) == 0);
