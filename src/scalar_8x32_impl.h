...
SECP256K1_INLINE static unsigned int secp256k1_scalar_get_bits(const secp256k1_scalar_t *a, int offset, int count) {
    VERIFY_CHECK((offset + count - 1) / 32 == offset / 32);
    return (a->d[offset / 32] >> (offset % 32)) & ((1 << count) - 1);
}

SECP256K1_INLINE static unsigned int secp256k1_scalar_get_bits_var(const secp256k1_scalar_t *a, int offset, int count) {
    VERIFY_CHECK(count < 32);
    VERIFY_CHECK(offset + count <= 256);
    if ((offset + count - 1) / 32 == offset / 32) {
        return secp256k1_scalar_get_bits(a, offset, count);
    } else {
        VERIFY_CHECK(offset / 32 + 1 < 8);
        return ((a->d[offset / 32] >> (offset % 32)) | (a->d[offset / 32 + 1] << (32 - (offset % 32)))) & ((((uint32_t)1) << count) - 1);
    }
}
...
static void secp256k1_scalar_add_bit(secp256k1_scalar_t *r, unsigned int bit) {
    ...
    uint64_t t = (uint64_t)r->d[0] + (((uint32_t)((bit / 32) == 0)) << bit);
    r->d[0] = t & 0xFFFFFFFFULL; t >>= 32;
    t += (uint64_t)r->d[1] + (((uint32_t)((bit / 32) == 1)) << (bit % 32));
    r->d[1] = t & 0xFFFFFFFFULL; t >>= 32;
    t += (uint64_t)r->d[2] + (((uint32_t)((bit / 32) == 2)) << (bit % 32));
    r->d[2] = t & 0xFFFFFFFFULL; t >>= 32;
    t += (uint64_t)r->d[3] + (((uint32_t)((bit / 32) == 3)) << (bit % 32));
    r->d[3] = t & 0xFFFFFFFFULL; t >>= 32;
    t += (uint64_t)r->d[4] + (((uint32_t)((bit / 32) == 4)) << (bit % 32));
    r->d[4] = t & 0xFFFFFFFFULL; t >>= 32;
    t += (uint64_t)r->d[5] + (((uint32_t)((bit / 32) == 5)) << (bit % 32));
    r->d[5] = t & 0xFFFFFFFFULL; t >>= 32;
    t += (uint64_t)r->d[6] + (((uint32_t)((bit / 32) == 6)) << (bit % 32));
    r->d[6] = t & 0xFFFFFFFFULL; t >>= 32;
    t += (uint64_t)r->d[7] + (((uint32_t)((bit / 32) == 7)) << (bit % 32));
