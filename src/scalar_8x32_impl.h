...
void static secp256k1_scalar_reduce_512(secp256k1_scalar_t *r, const uint32_t *l) {
    ...
    c = m0; o = 0;
    muladd(m8, SECP256K1_N_C_0);
    uint32_t p0; extract(p0);
    sumadd(m1);
    ...
    sumadd(m7);
    muladd(m12, SECP256K1_N_C_3);
    sumadd(m11);
    uint32_t p7; extract(p7);
    uint32_t p8 = c + m12;
    VERIFY_CHECK(p8 <= 2);
